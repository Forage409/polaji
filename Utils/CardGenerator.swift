...
class CardGenerator {
    ...
    func generate(templateId: String, inputs: [String: String]) -> GeneratedCard {
        ...
        case "rich_card":
            ...
            imageName = "tpl_rich"
            let richTitles = ["低调富豪观察报告", "隐藏财力鉴定卡", "来财体质检测中", "钱包深度可疑报告", "平民伪装失败现场", "低调但不完全低调", "疑似富豪体验生活", "花钱淡定指数报告", "嘴上没钱行为很贵", "隐藏资产雷达启动", "消费习惯异常样本", "富贵气息泄露记录"]
            ...
            let richLevels = ["轻度可疑", "低调玩家", "疑似有矿", "来财体质", "隐藏富豪", "平民伪装者", "预算外选手", "钱包深不可测"]
            ...
            stats = [StatItem(name: "财富掩饰力", value: Int.random(in: 60...100)), StatItem(name: "真实财力", value: Int.random(in: 60...100)), StatItem(name: "淡定指数", value: Int.random(in: 60...100))]
            evidenceList = Array(richEvidence.shuffled().prefix(3))
            
        case "single_card":
            templateType = "diagnostic"
            imageName = "tpl_single_transparent"
            let singleTitles = ["脱单可能性观察报告", "桃花信号检测中", "感情进度条加载中", "单身状态复盘卡", "恋爱气氛可疑报告", "脱单雷达已启动", "桃花运势轻量分析", "心动预警鉴定卡", "暧昧气息检测报告", "恋爱可能性说明书", "嘴硬单身观察样本", "被动心动研究报告"]
            ...
            let singleLevels = ["桃花待机中", "轻微心动", "暧昧预备役", "脱单有戏", "嘴硬选手", "被动心动型", "慢热但有机会", "缘分加载中"]
        ...
        case "stay_up":
            ...
            let stayUpTitles = ["深夜活动观察报告", "熬夜体质检测卡", "夜行动物鉴定书", "凌晨在线状态报告", "作息稳定崩坏记录", "黑眼圈浓度分析", "今晚又没早睡卡", "白天困晚上精神报告", "深夜清醒研究样本", "早睡失败复盘卡", "睡眠系统异常报告", "凌晨三点精神股东"]
            ...
            let stayUpLevels = ["轻度晚睡", "夜间活跃", "凌晨常驻", "作息漂移", "黑眼圈会员", "早睡困难户", "深夜清醒型", "白天省电模式"]
            ...
            stats = [StatItem(name: "凌晨活跃度", value: Int.random(in: 60...100)), StatItem(name: "拖延指数", value: Int.random(in: 60...100)), StatItem(name: "黑眼圈浓度", value: Int.random(in: 60...100))]
        ...
        case "group_judge":
            ...
            let judgeTitles = ["群聊临时判决书", "朋友局小法庭", "今日群聊审判", "奶茶责任认定书", "已读不回调查报告", "饭局责任裁定卡", "群友联合观察记录", "轻量级判官上线", "本群今日裁定", "气氛组责任书", "临时开庭通知", "群聊行为鉴定书"]
            ...
            let judgeCrimeTexts = ["已读不回", "关键时刻消失", "迟到但很淡定", "只发表情不说话", "装作没看见", "饭点突然失联", "嘴上说马上到", "说随便但很挑", "气氛到了人没到", "重要消息选择性失明"]
            let judgeShortVerdicts = ["请喝奶茶", "发个表情包", "下次请客", "当气氛组", "公开解释", "主动认领", "补发消息", "发句道歉", "负责点单", "接受调侃", "请大家吃糖", "今晚别潜水"]
        ...
        case "friend_vote":
            ...
            let voteTitles = ["朋友局临时排行榜", "今日群聊投票结果", "好友观察榜单", "谁最像榜单公布", "群友印象排行榜", "本局投票已出炉", "友情观察结果", "离谱但合理榜单", "群聊民意调查", "朋友局结果公示", "今日整活排行榜", "群友一致认为"]
        ...
        case "truth_dare":
            ...
            let truthDareLevels = ["轻松局", "有点心跳", "朋友局刚好", "轻微社死", "气氛担当", "勇敢一点", "适合热场", "别想太多"]
        ...
        default:
            ...
            let personaLevels = ["低电量但能用", "表面冷静型", "轻微发疯型", "稳定摸鱼型", "情绪价值不足", "活人感恢复中", "从容是假象", "连滚带爬但完成", "看似没事型", "今天先这样吧"]
