...
class CardGenerator {
    ...
    func generate(templateId: String, inputs: [String: String]) -> GeneratedCard {
        ...
        case "single_card":
            ...
            let singleTitles = ["脱单可能性观察报�?, "桃花信号检测中", "感情进度条加载中", "单身状态复盘卡", "恋爱气氛可疑报告", "脱单雷达已启�?, "桃花运势轻量分析", "心动预警鉴定�?, "暧昧气息检测报�?, "恋爱可能性说明书", "嘴硬单身观察样本", "被动心动研究报告"]
            ...
            stats = [StatItem(name: "心动阈�?, value: Int.random(in: 60...100)), StatItem(name: "被动指数", value: Int.random(in: 60...100)), StatItem(name: "桃花潜能", value: Int.random(in: 60...100))]
            evidenceList = Array(singleEvidence.shuffled().prefix(3))
            
        case "stay_up":
            templateType = "diagnostic"
            imageName = "tpl_stay_up"
            let stayUpTitles = ["深夜活动观察报告", "熬夜体质检测卡", "夜行动物鉴定�?, "凌晨在线状态报�?, "作息稳定崩坏记录", "黑眼圈浓度分�?, "今晚又没早睡�?, "白天困晚上精神报�?, "深夜清醒研究样本", "早睡失败复盘�?, "睡眠系统异常报告", "凌晨三点精神股东"]
            ...
            let stayUpLevels = ["轻度晚睡", "夜间活跃", "凌晨常驻", "作息漂移", "黑眼圈会�?, "早睡困难�?, "深夜清醒�?, "白天省电模式"]
            ...
            stats = [StatItem(name: "凌晨活跃�?, value: Int.random(in: 60...100)), StatItem(name: "拖延指数", value: Int.random(in: 60...100)), StatItem(name: "黑眼圈浓�?, value: Int.random(in: 60...100))]
            evidenceList = Array(stayUpEvidence.shuffled().prefix(3))
            
        case "boss_card":
            templateType = "diagnostic"
            imageName = "tpl_boss_transparent"
            let bossTitles = ["老板气质检测报�?, "领导感自动识别中", "开会人设鉴定卡", "气场管理观察报告", "朋友局老板样本", "总结型人格分�?, "画饼能力检测卡", "气质压迫感报�?, "群聊管理者说明书", "疑似老板行为记录", "发言含金量分�?, "会议感溢出警�?]
            ...
            let bossLevels = ["轻度领导�?, "朋友局主理�?, "会议感溢�?, "气场稳定�?, "总结型人�?, "画饼预备�?, "决策担当", "疑似老板气质"]
            ...
            stats = [StatItem(name: "控场能力", value: Int.random(in: 60...100)), StatItem(name: "总结�?, value: Int.random(in: 60...100)), StatItem(name: "气场指数", value: Int.random(in: 60...100))]
        ...
        case "group_judge":
            ...
            let judgeTitles = ["群聊临时判决�?, "朋友局小法�?, "今日群聊审判", "奶茶责任认定�?, "已读不回调查报告", "饭局责任裁定�?, "群友联合观察记录", "轻量级判官上�?, "本群今日裁定", "气氛组责任书", "临时开庭通知", "群聊行为鉴定�?]
            ...
            let judgeCrimeTexts = ["已读不回", "关键时刻消失", "迟到但很淡定", "只发表情不说�?, "装作没看�?, "饭点突然失联", "嘴上说马上到", "说随便但很挑", "气氛到了人没�?, "重要消息选择性失�?]
            let judgeShortVerdicts = ["请喝奶茶", "发个表情�?, "下次请客", "当气氛组", "公开解释", "主动认领", "补发消息", "发句道歉", "负责点单", "接受调侃", "请大家吃�?, "今晚别潜�?]
            ...
            subtitle = "被告人：\(defendantInput.isEmpty ? "某神秘群�? : defendantInput)"
        ...
        case "friend_vote":
            ...
            let voteTitles = ["朋友局临时排行�?, "今日群聊投票结果", "好友观察榜单", "谁最像榜单公�?, "群友印象排行�?, "本局投票已出�?, "友情观察结果", "离谱但合理榜�?, "群聊民意调查", "朋友局结果公示", "今日整活排行�?, "群友一致认�?]
            ...
            let rankLabels = ["TOP 1 断层领先", "TOP 2 稳定上榜", "TOP 3 压线入围", "隐藏选手", "气氛担当", "争议候�?]
            ...
            if shuffledParticipants.isEmpty { shuffledParticipants = ["路人�?, "路人�?, "路人�?] }
        ...
        case "truth_dare":
            ...
            let truthDareLevels = ["轻松局", "有点心跳", "朋友局刚好", "轻微社死", "气氛担当", "勇敢一�?, "适合热场", "别想太多"]
            let failurePunishments = ["下一轮必须先选�?, "发一个表情包认领失败�?, "给大家补一句解释�?, "下一题不能跳过�?, "负责带动下一轮气氛�?, "请朋友帮你选下一题�?, "用一句话总结失败感想�?, "给自己一个台阶：我只是太谨慎了�?]
            
            let isTruth = mode == "真心�? || (mode == "随机" && Bool.random())
            title = isTruth ? "真心话时�? : "大冒险时�?
            subtitle = isTruth ? "坦诚相待局" : "行动力测�?
            ...
            resultLevel = isTruth ? "勇敢说真�? : "果断去执�?
            finalComment = tonePrefix + "大家都在看着呢，别想轻易蒙混过关�?
            
        default:
            templateType = "diagnostic"
            imageName = "icon_persona"
            let personaTitles = ["今日人设�?, "今日状态说明书", "今日精神面貌", "今日社交电量", "今日生活模式", "今日气质检�?, "今日状态条", "今日角色加载�?, "今日人类观察报告", "今日情绪说明", "今日生存模式", "今日整活身份"]
            let personaLevels = ["低电量但能用", "表面冷静�?, "轻微发疯�?, "稳定摸鱼�?, "情绪价值不�?, "活人感恢复中", "从容是假�?, "连滚带爬但完�?, "看似没事�?, "今天先这样吧"]
            ...
            subtitle = "\(nickname) 的今日诊�?
            resultLevel = personaLevels.randomElement() ?? " \
            finalComment = tonePrefix + personaFinals.randomElement() ?? " \
            stats = [StatItem(name: "社交电量", value: Int.random(in: 10...50)), StatItem(name: "活人�?, value: Int.random(in: 10...80)), StatItem(name: "伪装从容", value: Int.random(in: 60...100))]
